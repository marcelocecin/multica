...
Each AI coding tool accepts `MULTICA_<PROVIDER>_PATH` and `MULTICA_<PROVIDER>_MODEL` to override the command path and default model. QwenPaw and Prime Agent are the exceptions: neither has a `_MODEL` variable (`MULTICA_QWENPAW_MODEL` / `MULTICA_PRIME_MODEL`), because Multica never sends either of them a model — see [AI coding tools comparison](/providers#model-sources). Machine-wide default arguments via `MULTICA_<PROVIDER>_ARGS` are currently supported for six tools: Claude Code, Codex, CodeBuddy, Qwen Code, QwenPaw, and Prime Agent. The variables are `MULTICA_CLAUDE_ARGS`, `MULTICA_CODEX_ARGS`, `MULTICA_CODEBUDDY_ARGS`, `MULTICA_QWEN_ARGS`, `MULTICA_QWENPAW_ARGS`, and `MULTICA_PRIME_ARGS`. For example:
