...
Each AI coding tool accepts `MULTICA_<PROVIDER>_PATH` and, when model overrides are supported, `MULTICA_<PROVIDER>_MODEL`. QwenPaw, MiniMax Code, and Prime Agent have no model variable because Multica never sends them a model; MiniMax Code's path variable is `MULTICA_MCODE_PATH` — see [AI coding tools comparison](/providers#model-sources). DeepSeek Harness supports `MULTICA_DSH_PATH` and `MULTICA_DSH_MODEL` (a model id from the dsh catalog, e.g. `deepseek-official/deepseek-chat`). Machine-wide default arguments via `MULTICA_<PROVIDER>_ARGS` are currently supported for six tools: Claude Code, Codex, CodeBuddy, Qwen Code, QwenPaw, and Prime Agent. The variables are `MULTICA_CLAUDE_ARGS`, `MULTICA_CODEX_ARGS`, `MULTICA_CODEBUDDY_ARGS`, `MULTICA_QWEN_ARGS`, `MULTICA_QWENPAW_ARGS`, and `MULTICA_PRIME_ARGS`. For example:
